use super::store::SymbologyStore;
use crate::{
    grpc::GrpcClientConfig,
    synced::{SyncHandle, Synced},
};
use anyhow::{bail, Result};
use api::{
    grpc::json_service::symbology_v2_client::SymbologyV2Client as SymbologyV2GrpcClient,
    symbology_v2::protocol::{SubscribeSymbologyV2, SymbologyUpdate},
    utils::sequence::SequenceIdAndNumber,
};
use log::{debug, trace};
use tonic::{
    transport::{Channel, Endpoint},
    Streaming,
};
use url::Url;

pub struct SymbologyV2Client {
    grpc_endpoint: Endpoint,
    grpc_config: GrpcClientConfig,
    grpc: SymbologyV2GrpcClient<Channel>,
    upstream_seqno: Option<SequenceIdAndNumber>,
    updates: Streaming<SymbologyUpdate>,
    pub store: SymbologyStore,
    pub ready: SyncHandle<bool>,
}

#[derive(Clone)]
pub struct SymbologyV2ClientHandle {
    pub store: SymbologyStore,
    pub ready: Synced<bool>,
}

impl SymbologyV2Client {
    pub async fn connect(url: &Url, grpc_config: GrpcClientConfig) -> Result<Self> {
        let endpoint = Endpoint::try_from(url.to_string())?;
        debug!("reconnecting to {}...", endpoint.uri());
        let channel = grpc_config.connect_to(endpoint.clone()).await?;
        let mut grpc = SymbologyV2GrpcClient::new(channel);
        debug!("subscribing to symbology_v2 updates...");
        let updates =
            grpc.subscribe_symbology_v2(SubscribeSymbologyV2 {}).await?.into_inner();
        Ok(Self {
            grpc_endpoint: endpoint,
            grpc_config,
            grpc,
            upstream_seqno: None,
            updates,
            store: SymbologyStore::new(),
            ready: SyncHandle::new(false),
        })
    }

    pub async fn reconnect(&mut self) -> Result<()> {
        debug!("reconnecting to {}...", self.grpc_endpoint.uri());
        let channel = self.grpc_config.connect_to(self.grpc_endpoint.clone()).await?;
        self.grpc = SymbologyV2GrpcClient::new(channel);
        debug!("subscribing to symbology_v2 updates...");
        self.store.clear();
        self.upstream_seqno = None;
        self.updates =
            self.grpc.subscribe_symbology_v2(SubscribeSymbologyV2 {}).await?.into_inner();
        Ok(())
    }

    pub fn handle(&self) -> SymbologyV2ClientHandle {
        SymbologyV2ClientHandle { store: self.store.clone(), ready: self.ready.synced() }
    }

    pub async fn next(&mut self) -> Result<()> {
        match self.updates.message().await {
            Err(e) => bail!("error reading from stream: {:?}", e),
            Ok(None) => bail!("symbology updates stream ended"),
            Ok(Some(update)) => {
                trace!("received update: {:?}", update);
                if let Some(upstream_seqno) = self.upstream_seqno {
                    if !update.sequence.is_next_in_sequence(&upstream_seqno) {
                        bail!(
                            "skipped sequence number: {} -> {}",
                            upstream_seqno,
                            update.sequence
                        );
                    }
                }
                self.upstream_seqno = Some(update.sequence);
                self.store.apply_update(update)?;
                self.ready.set(true);
            }
        }
        Ok(())
    }
}
